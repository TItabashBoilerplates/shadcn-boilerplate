...
import {
	...
	planDefinitions,
} from "../frontend/packages/polar/plans";
...
async function syncProduct(
	...
	if (dryRun) {
		console.log(
			`  [DRY RUN] Would create product with ${definition.prices.length} price(s)`,
		);
	...
	try {
		...
		if (response.prices) {
			for (let i = 0; i < response.prices.length; i++) {
				...
				const interval =
					defPrice.type === "recurring" ? defPrice.interval : "one_time";
