...
```bash
./scripts/incus/devbox.sh doctor    # 前提の診断だけ
./scripts/incus/devbox.sh up        # 箱の作成〜マウントまで（冪等）
./scripts/incus/devbox.sh shell     # 箱に入る。cd 済みで direnv が発火する
...
```bash
./scripts/incus/devbox.sh status    # IP と各サービスの URL
./scripts/incus/devbox.sh exec ci-check
./scripts/incus/devbox.sh stop
./scripts/incus/devbox.sh destroy   # 作業ツリーはホスト側なので失われない
```
...
```bash
DEVBOX_NAME=shadcn-feature-a ./scripts/incus/devbox.sh up
```
...
| 変数 | 既定 | 用途 |
|---|---|---|
| `DEVBOX_NAME` | リポジトリのディレクトリ名 | インスタンス名 |
| `DEVBOX_IMAGE` | `images:debian/13/cloud` | ベースイメージ |
| `DEVBOX_CPU` / `DEVBOX_MEMORY` | `8` / `16GiB` | リソース上限 |
| `DEVBOX_LOCAL_PATHS` | `frontend/node_modules drizzle/node_modules backend-py/.venv .devenv .direnv` | ホストと共有せず箱側の FS に置くパス |
