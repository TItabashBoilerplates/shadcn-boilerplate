...
- **調査日**: 2026-08-22
- **対象**: Vercel Services / Container Images（`backend-py/` 直下の Dockerfile）
- **関連**: 配置・名前・ビルドコンテキストは [2026-08-22-vercel-services-container-build-context.md](./2026-08-22-vercel-services-container-build-context.md)
...
```bash
cd backend-py
docker build -f Dockerfile.vercel -t api-vercel .
...
| 何 | どこ |
|---|---|
| 実装の修正（PORT 8080 / 絶対パス CMD）と**理由のコメント** | `backend-py/Dockerfile.vercel` |
