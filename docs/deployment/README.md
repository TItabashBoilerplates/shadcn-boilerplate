...
| backend が 502 / タイムアウト | サーバが `0.0.0.0:$PORT` で listen しているか、**Vercel project の env `PORT` がコンテナの値（8080）と一致**しているかを確認（`infra-bootstrap vercel` が自動投入する）。Doppler→Vercel(backend) で外部 API キーが、Marketplace 連携で `SUPABASE_URL` / `POSTGRES_URL` が届いているかも確認。 |
