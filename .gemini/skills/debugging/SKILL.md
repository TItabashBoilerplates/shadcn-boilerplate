...
### 非対話環境（CI / Claude Code / Gemini エージェント）
...
> Supabase は Supabase CLI が Docker コンテナを所有。devenv の `supabase:start` task が backend の `before` に登録されているため `devenv up` 起動時は Supabase → backend の順で立ち上がる。
