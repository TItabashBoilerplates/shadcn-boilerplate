...
## Storybook と Maestro の UI テストは層が違う（どちらも要る）

「UI テスト」という言葉が 2 か所に出てくるので、担当範囲を明確にしておく。

| | **Storybook** | **Maestro の `ui` スイート** |
|---|---|---|
| 単位 | **コンポーネント 1 つ** | **画面 1 枚（実ブラウザ / 実機）** |
| 見るもの | 見た目・状態の網羅（初期 / ローディング / エラー / 空 / 無効） | 画面が組み上がっているか・必須の導線があるか・遷移できるか |
| 依存 | 無し（props で状態を作る） | 実際にアプリを起動する（バックエンドには**書き込まない**） |
| 例 | `LoginForm` のエラー表示 | ログイン画面に「パスワードをお忘れですか？」があり、そこへ遷移できる |
| 検知できないこと | ルーティング・i18n の配線・レイアウトの被り | コンポーネント単体の細かい状態 |

Storybook が緑でも「配線が外れて画面に出ていない」は起きる。逆に Maestro だけだと
「どのコンポーネントのどの状態が壊れたのか」が分からない。**両方揃えること。**

→ Maestro 側は `.claude/skills/maestro/` と `.maestro/README.md`
