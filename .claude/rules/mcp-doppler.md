...
## 現在のフェーズ（← 書き込み前に必ず `PROJECT.md` の `doppler_phase` を確認する）

フェーズは **`PROJECT.md` の frontmatter `doppler_phase`** で宣言する。このファイル（スタック層）には
書かない。スタック層に書くと雛形の更新でプロジェクトの判断が巻き戻る。

| `doppler_phase` | 意味 |
|---|---|
| `full-access`（初期構築） | 実ユーザーがまだいない構築フェーズ。高速イテレーション優先で、エージェントは **全 config（prd 含む）のシークレットを作成・更新してよい** |
| `protected`（本番） | ユーザーが稼働し始めたら切り替える保護フェーズ。`prd` への書き込みは **明示承認制**になる（後述） |

**`PROJECT.md` に無い・読めない・値が上記以外の場合は `protected` として扱う**（安全側に倒す）。
...
1. **宣言の更新**: `PROJECT.md` の `doppler_phase` を `protected` に変更（エージェントの挙動が変わる）。
