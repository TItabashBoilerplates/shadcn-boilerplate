...
| 方針 | 向いているケース | 必須の担保 |
|---|---|---|
| **【既定】応答は返し、記録失敗はログ + リトライ** | 社内コスト把握、使用量表示 | 失敗件数の監視。**黙って捨てない** |
| 記録できなければ応答も失敗させる | 従量課金・クレジット制（記録漏れ = 売上欠損） | 記録の高可用性、リトライ |

**既定は前者**。記録の失敗でユーザーの機能まで落とすのは、課金していない段階では割に合わない。
課金を始める時点で後者へ切り替える（切り替えは記録経路が 1 か所に集約されていれば容易）。
...
- 並行リクエストで上限を超える（チェックと確定の間に他のリクエストが走る）。厳密にやるなら
  ...
  **既定は 2 相にしない**（多少の超過を許容する）。上限は「暴走を止める安全弁」であって
  会計上の厳密な締めではないので、1 リクエスト分の超過を防ぐために全リクエストへ
  予約処理を入れるのは割に合わない。**前払いクレジット制で残高が実際のお金である場合だけ** 2 相にする。
