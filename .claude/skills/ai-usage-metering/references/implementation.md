...
| 方針 | 向いているケース | 必須の担保 |
|---|---|---|
| 応答は返し、記録失敗はログ + リトライ | 社内コスト把握、使用量表示（記録漏れが売上に直結しない） | 失敗件数の監視。**黙って捨てない** |
| 記録できなければ応答も失敗させる | 従量課金・クレジット制（記録漏れ = 売上欠損） | 記録の高可用性、リトライ |

**どちらかは「そのサービスで記録漏れが何を意味するか」で決まる。**
課金していないなら記録失敗でユーザーの機能まで落とすのは割に合わないし、
従量課金なら記録漏れはそのまま損失になる。**課金の有無が読み取れないなら開発者に確認する。**

これは可逆な判断なので、記録経路をラッパ 1 か所に集約しておけば後から切り替えられる。
...
- 並行リクエストで上限を超える（チェックと確定の間に他のリクエストが走る）。厳密にやるなら
  **予約（reserve）→ 確定（commit）→ 差分返却**の 2 相にする。
  **どこまで厳密にするかはサービス次第**。上限が「暴走を止める安全弁」なら多少の超過を許容してよく、
  **前払いクレジット制で残高が実際のお金**なら 2 相にする必要がある。
  料金プランの性質から読み取れないなら**開発者に確認する**（超過を許すかは事業判断）。
...
**保存は USD のまま**（プロバイダの請求が USD 建てなので、円で保存すると突合できなくなる）。
レートの選び方・取得元（無料 API）・丸め・税務上の継続適用は
**`references/currency-jpy.md`** にまとめてあるので、円を扱うならそちらを読む。

要点だけ: **社内速報は ECB 参照レートの日次、会計確定は銀行 TTM の月初固定**という二段構えにし、
確定値は換算結果を凍結する（表示のたびに換算すると先月の金額が動く）。
