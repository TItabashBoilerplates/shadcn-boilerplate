...
| **B. イベント + metric 内訳の 2 テーブル**【既定】 | 生成 AI 全般。画像・音声・動画・ツール課金が混ざりうる場合 | JOIN が 1 段増える |
...
| 方式 | 向き | 注意 |
|---|---|---|
| **【既定】定期ジョブ（`pg_cron` / スケジュール Edge Function）で前日分を再集計** | まずこれ。単価修正や `price_missing` の再計算もこの経路で通る | 当日分は最新にならない |
