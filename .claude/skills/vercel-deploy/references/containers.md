...
対象: `backend-py/vercel.json` の `services` + `apps/<app>/Dockerfile.vercel`。
...
| # | 確認 |
|---|---|
| 1 | `apps/<app>/Dockerfile.vercel` を作り、`vercel.json` の `services` と `rewrites` に足したか |
...
- [Vercel REST API: Get logs for a deployment](https://vercel.com/docs/rest-api/logs/get-logs-for-a-deployment)
- 本リポジトリ: `backend-py/apps/api/Dockerfile.vercel`（冒頭コメントが正本）/
