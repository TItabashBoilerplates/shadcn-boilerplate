...
> **併読必須**: モデルを呼ぶ実装を追加・変更するときは `.claude/skills/ai-usage-metering/` も起動すること。
> トークン使用量とコストの記録は後付けできない（過去のイベントは作れない）ため、実装とセットで入れる。
> エージェントは 1 リクエストで何十回もモデルを呼ぶので、`trace_id` を束縛した callback での一元記録が要る。
