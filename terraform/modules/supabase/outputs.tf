...
output "migrate_postgres_urls" {
  description = <<-EOT
    環境名 → Drizzle migration（GitHub Actions）用の接続文字列（Doppler の MIGRATE_POSTGRES_URL）。
  ...
  EOT
  value       = local.migrate_postgres_urls
  sensitive   = true
}

output "migrate_postgres_url_envs" {
  description = <<-EOT
    session pooler の接続先を解決できた環境名のリスト。
    migrate_postgres_urls は sensitive なので for_each に使えない（Terraform の制約）。
