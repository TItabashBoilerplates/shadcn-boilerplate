...
**認証・認可はアプリ（マイクロフロントエンド）ごとに分離する。** その分離は「アプリごとに認証スタック自体を分ける」方針で行う — メイン（web）= Supabase Auth、管理者（admin）= **Better Auth**。別システム・別 cookie なので単一ドメイン合成でも自然に分離される（Supabase Auth 単独でアプリ間分離することは基本しない）。**共有するのはコード（packages）であって、認証セッションではない。** 詳細は [マイクロフロントエンド運用ガイド](./microfrontends.md) を参照。
