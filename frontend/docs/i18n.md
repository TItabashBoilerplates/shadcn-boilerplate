...
├── src/                       # FSD レイヤー
│   ├── app/                   # FSD Application レイヤー
│   │   ├── providers/         # グローバルプロバイダー
│   │   └── styles/            # グローバルスタイル
│   ├── views/                 # FSD Views レイヤー（Next.js Pages Router と競合しないよう views に命名）
│   │   └── home/
│   └── shared/
│       ├── config/
│       │   └── i18n/
...
│               └── index.ts      # パブリック API
└── proxy.ts                   # Next.js Proxy (Next.js 16+)
```
...
- ✅ `src/views/` - FSD の Views レイヤー
- ❌ `src/pages/` - Next.js Pages Router として認識されるため使用不可
- ✅ `app/` - Next.js App Router（プロジェクトルート）
- ✅ `src/app/` - FSD Application レイヤー
